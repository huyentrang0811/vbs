...
def _initial_Bernoulli_fn_wrapper(prior_prob):
    #print(f"Use Bernoulli distribution with p(s=1) = {tf.keras.backend.get_value(tf.convert_to_tensor(prior_prob))}.")
    def _initial_Bernoulli_fn(dtype, shape, name, 
                                        trainable, add_variable_fn):
        del name, trainable, add_variable_fn   # unused
        dist = tfd.Bernoulli(
            probs=tf.keras.backend.get_value(tf.convert_to_tensor(prior_prob)[1])*tf.ones(shape, dtype), 
            dtype=dtype)
        batch_ndims = tf.size(dist.batch_shape_tensor())
        return tfd.Independent(
            dist, reinterpreted_batch_ndims=batch_ndims)
    return _initial_Bernoulli_fn

def ind_Bernoulli_fn(prior_prob_factor, prob=None):
    if prob is not None:
        return _initial_Bernoulli_fn_wrapper(prob)
    else:
        # use multivariate normal prior with specified prior variance
        single_prob1 = tf.concat([tf.math.sigmoid(tf.convert_to_tensor([prior_prob_factor])), tf.math.sigmoid(-tf.convert_to_tensor([prior_prob_factor]))], 0)
        return _initial_Bernoulli_fn_wrapper(single_prob1)
...
def default_Bernoulli_fn(
        probs_init_factor=tf.initializers.random_uniform(minval=-0.5, maxval=0.5),
        probs_regularizer=None,
        probs_constraint=None):
    """Creates Bernoulli distribution.
    Args:
        dtype: Type of parameter's event.
        shape: Python `list`-like representing the parameter's event shape.
        name: Python `str` name prepended to any created (or existing)
        ...
        access existing) `tf.Variable`s.
    Returns:
        a function return Bernoulli distribution.
    """
    def _fn(dtype, shape, name, trainable, add_variable_fn):
        probs_factor = add_variable_fn(
            name=name + '_probs_factor',
            shape=(shape[0],),
            initializer=probs_init_factor,
            regularizer=probs_regularizer,
            constraint=probs_constraint,
            dtype=dtype,
            trainable=trainable)
        dist = bernoulli_lib.Bernoulli(probs=tf.ones(shape=shape)*tf.keras.backend.get_value(tf.math.sigmoid(tf.convert_to_tensor(probs_factor))), dtype=dtype)
        batch_ndims = tf.size(dist.batch_shape_tensor())
...
def GumbelSoftMax_fn(
    ...
    def _fn(dtype, shape, name, trainable, add_variable_fn):
        probs_factor = add_variable_fn(
            ...
            dtype=dtype,
            trainable=trainable)
        single_prob1 = tf.concat([tf.math.sigmoid(tf.convert_to_tensor(probs_factor)), tf.math.sigmoid(-tf.convert_to_tensor(probs_factor))], 0)
        single_prob1_reshape = tf.reshape(single_prob1, [2, shape[0]])
        single_prob1_reshape_transpose=tf.transpose(single_prob1_reshape)
        dist = tfp.distributions.RelaxedOneHotCategorical(
            temperature=0.01,
            probs=single_prob1_reshape_transpose)
        batch_ndims = tf.size(dist.batch_shape_tensor())
        return independent_lib.Independent(
            dist, reinterpreted_batch_ndims=batch_ndims)
...
def KLDivergenceOfCorrespondingBernoulli(posterior, prior, posterior_tensor):
    del posterior_tensor  # not used
    q = posterior.distribution.probs[:, 0]
    p = prior.distribution.probs
    return tf.reduce_mean(q*tf.log(q/p) + (1.0-q) * tf.log((1.0-q)/(1.0-p)))
